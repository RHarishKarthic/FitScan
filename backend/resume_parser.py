...
# ── Step 2.5: Extract Name & Email ──────────────────────────────────────────

TITLE_EXCLUSIONS = [
    "engineer", "developer", "architect", "lead", "manager", "analyst",
    "consultant", "administrator", "designer", "specialist", "intern",
    "curriculum vitae", "resume", "profile", "summary", "experience",
...
def extract_name(text, basename=""):
    lines = text.split('\n')
    # Look at first 10 non-empty lines for a name
    for line in lines[:10]:
        ...
        # Skip if looks like email, phone, or title keyword
        if "@" in clean_line or re.search(r'\d', clean_line) or line_lower in [s.lower() for s in SKILL_VOCAB]:
            continue
        if any(re.search(r'\b' + re.escape(ex) + r'\b', line_lower) for ex in TITLE_EXCLUSIONS):
            continue
