...
def format_candidate(resume_data, jd_data, match_result, insights):
    ...
    email = resume_data.get("email") or f"{first_name}@example.com"
...
async def upload_jd_text(payload: JDTextPayload):
    ...
    # Write text to a unique temp file so parse_jd can process it normally
    temp_path = os.path.join(UPLOAD_DIR, f"jd_{uuid.uuid4().hex[:8]}.txt")
